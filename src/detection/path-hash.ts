...
export function entriesWithinPath(entries: GitTreeEntry[], monitoredPath: string): GitTreeEntry[] {
  return entries
    .filter((entry) => entry.type === undefined || entry.type === "blob")
    .map((entry) => {
      const relative = relativeTreePath(entry.path, monitoredPath);
      return relative === undefined || relative === "" ? undefined : { ...entry, path: relative };
    })
 ...
 */
export function hashTreeEntries(entries: GitTreeEntry[], monitoredPath: string): string {
  const hash = createHash("sha256");
  for (const entry of entriesWithinPath(entries, monitoredPath)) {
...
export function changedTreePaths(previous: GitTreeEntry[], next: GitTreeEntry[], monitoredPath: string): string[] {
  const before = new Map(entriesWithinPath(previous, monitoredPath).map((entry) => [entry.path, `${entry.sha}:${entry.mode}`]));
  const after = new Map(entriesWithinPath(next, monitoredPath).map((entry) => [entry.path, `${entry.sha}:${entry.mode}`]));
